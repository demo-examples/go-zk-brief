...
func delserver(w http.ResponseWriter, r *http.Request) {
	...
	zkidcs := r.Form["zkidc"]
	serverKeys := r.Form["serverKey"]

	input := fmt.Sprintf("keys:%v, zkidcs:%v, destNames:%v, serverKey:%v", keys, zkidcs, destNames, serverKeys)
	api := "delserver"
	defer handleError(w, input, api)


	// 参数检验
	checkParams(keys, destNames, zkidcs, serverKeys)
	// 判断key是否正确
	checkKeys(keys[0])

	checkServerKey(serverKeys[0])

	c := ZkConns[zkidcs[0]]


	zkServerPath := Conf.ZkPath + "/" + destNames[0] + "/" + serverKeys[0]
